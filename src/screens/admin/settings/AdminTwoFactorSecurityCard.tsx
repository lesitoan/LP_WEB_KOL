...
} from "@/services/api/admin/authApi";
import { useAdminAuthSession } from "@/hooks/admin/useAdminAuthSession";
import type { AdminTwoFactorSetupResult } from "@/types/admin/auth";

type AdminTwoFactorSecurityCardProps = {
  enabled?: boolean;
};

type DialogMode = "enable" | "disable";

export function AdminTwoFactorSecurityCard({ enabled }: AdminTwoFactorSecurityCardProps) {
  const { logout } = useAdminAuthSession();
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<DialogMode>(enabled ? "disable" : "enable");
  const [code, setCode] = useState("");
  const [codeError, setCodeError] = useState("");
  const [setupResult, setSetupResult] = useState<AdminTwoFactorSetupResult | null>(null);
  const [setupTwoFactor, { isLoading: isSettingUp }] = useSetupAdminTwoFactorMutation();
  const [enableTwoFactor, { isLoading: isEnabling }] = useEnableAdminTwoFactorMutation();
  const [disableTwoFactor, { isLoading: isDisabling }] = useDisableAdminTwoFactorMutation();
  ...
  return (
    <>
      <Card className="max-w-xl bg-surface-1 border-border">
        <CardHeader>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <SixDigitCodeInput
                idPrefix={isEnableMode ? "enableAdminTwoFactorCode" : "disableAdminTwoFactorCode"}
                value={code}
                onChange={(value) => {
                  setCode(value);
