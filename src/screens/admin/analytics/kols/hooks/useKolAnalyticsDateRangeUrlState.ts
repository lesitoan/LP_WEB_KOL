...
const KOL_ANALYTICS_DATE_RANGE_INITIAL_VALUES = {
  startDate: '',
  endDate: '',
}
...
export function useKolAnalyticsDateRangeUrlState() {
  const { values, setMany } = useUrlFilterState({
    initialValues: KOL_ANALYTICS_DATE_RANGE_INITIAL_VALUES,
  })
  ...
  const dateRange = useMemo<KolAnalyticsDateRange>(() => {
