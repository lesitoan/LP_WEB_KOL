      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-[calc(100vw-1rem)] max-w-[810px] gap-0 overflow-hidden border-border bg-surface-card p-0 shadow-2xl sm:max-w-[810px] [&_[data-slot=dialog-close]]:right-6 [&_[data-slot=dialog-close]]:top-4 [&_[data-slot=dialog-close]]:rounded-md [&_[data-slot=dialog-close]]:bg-surface-control [&_[data-slot=dialog-close]]:p-1.5 [&_[data-slot=dialog-close]]:text-zinc-300">
          <DialogHeader className="border-b border-border px-4 py-4 sm:px-6">
            <DialogTitle className="pr-10 text-lg font-medium text-foreground">Tổng quan nhóm</DialogTitle>
          </DialogHeader>
          <div className="max-h-[calc(92vh-61px)] overflow-y-auto overflow-x-hidden px-4 py-4 sm:px-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            {isFetching ? (
              <div className="flex min-h-[180px] items-center justify-center gap-2 text-sm font-normal text-muted-foreground">
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-border-strong p-4">
                  <h4 className="text-sm font-normal text-zinc-300">Thông tin nhóm</h4>
                  <div className="mt-4 grid gap-4 text-sm font-normal sm:grid-cols-2">
                    <p className="min-w-0 break-all text-foreground"><span className="text-xs font-normal text-muted-foreground">Tên nhóm:</span> {data.group.title}</p>
                    ...
                    <p className="text-foreground"><span className="text-xs font-normal text-muted-foreground">Volume tối thiểu:</span> {data.group.minVolumeRequired !== null ? `${formatVnd(data.group.minVolumeRequired)} VNĐ` : '---'}</p>
                    <p className="text-foreground"><span className="text-xs font-normal text-muted-foreground">Volume tối đa:</span> {data.group.maxVolumeRequired !== null ? `${formatVnd(data.group.maxVolumeRequired)} VNĐ` : '---'}</p>
                  </div>
                </div>

                <div className="rounded-lg border border-border-strong p-4">
                  <h4 className="text-sm font-normal text-zinc-300">Tóm tắt truy cập</h4>
                  <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    {summaryEntries.map(([label, value]) => (
                      <div key={String(label)} className="min-w-0 rounded-lg border border-border-strong p-4">
                        <p className="break-words text-xs font-normal text-muted-foreground">{label}</p>
                        <p className="break-all text-sm font-normal text-foreground">{value ?? '---'}</p>
                      </div>
                    ))}
                  </div>
                </div>
